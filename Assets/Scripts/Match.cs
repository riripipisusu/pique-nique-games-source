...
public enum GameId { Croque, Blackjack, Roulette, Quiz }
...
{
    public static string Name(GameId g) => g == GameId.Croque ? "Croque-Carotte" : g == GameId.Blackjack ? "Blackjack" : g == GameId.Roulette ? "Roulette française" : "Quiz d'images";

    public static string OptionName(GameId g, int option) =>
        g == GameId.Croque ? (option == 0 ? "Classique" : "Amélioré") : g == GameId.Blackjack ? option + " manches" : g == GameId.Roulette ? option + " coups" : new[] { "Flou", "Pixelisé", "Mélangé" }[option];
    ...
    public static IMatch Create(GameId g, int option, IList<string> names, int seed) =>
        ...
        : (IMatch)new Quiz(names, option, seed, Quiz.Pool);

    public static int MaxPlayers(GameId g) => g == GameId.Quiz ? Quiz.MaxPlayers : Rules.MaxPlayers;
}
